#----------------------------------------------------------------
# Figure 1, Panel A: Cross-sectional heterogeneity in WNS effects.
#
# This script runs regressions of bond spreads on WNS for different
# subsamples (all, high/low bats at risk, high/low assessment frequency),
# then creates an event study plot 
#----------------------------------------------------------------
...
low_bats <- feols(statetaxSPREAD_MMAinterp ~ wns
                  ...
                  + q1mintemp + q2mintemp + q3mintemp + q4mintemp
                  + q1precip + q2precip + q3precip + q4precip
                  | identif + year + agg_rating_min_lt, cluster ~ identif + year,
                  fixef.rm = "singleton",
                  data = spread %>% filter(high_bats == 0))

all <- feols(statetaxSPREAD_MMAinterp ~ wns
             + cum_outbreak_count
             + lnsize
             + lnmat
             + BQ
...
bar <- ggplot(results, aes(x = Name, y = Estimate)) +
  ...
  labs(
    y = "Bond Spread, basis points",
    x = NULL
  ) +
  scale_y_continuous(
    limits = c(-15, max(results$CI_high) + 5.5),
    breaks = c(-15, 0, 5, 11.47, 15, 20)
  ) +
...
ggsave(paste0('../../tables_figures_main/', 'figure1_panel_a.pdf'), bar,

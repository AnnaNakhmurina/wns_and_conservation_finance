#----------------------------------------------------------------
# Table SI-14 Panel B: Per capita property tax and own-source revenue regressions
# for rural counties in WNS-affected states. Outputs a LaTeX table.
#----------------------------------------------------------------
...
wns_states <- data %>% filter(wns != 0) %>% dplyr::select(STATEFP) %>% distinct()

data <- data %>% filter(STATEFP %in% wns_states$STATEFP)
...
etable(property_tax_1, property_tax,
       ...
       file = paste0('../../tables_figures_SI/table_SI-14_panel_b.tex'),
       replace = T,

#----------------------------------------------------------------
# Table SI-8 Panel A: Bond spread regressions using only WNS-positive counties
# as treated (excluding suspect counties). Progressively adds controls to estimate
# treatment effects on municipal bond spreads.
#----------------------------------------------------------------
...
m1 <-  feols(statetaxSPREAD_MMAinterp ~ only_wns_positive
             |identif + year , cluster ~ identif,
             data = spread
)
summary(m1)

m2 <-  feols(statetaxSPREAD_MMAinterp ~ only_wns_positive
             + cum_outbreak_count
...
m3 <- feols(statetaxSPREAD_MMAinterp ~ only_wns_positive
            + lnsize
            + lnmat
            + cum_outbreak_count
            + q1mintemp + q2mintemp + q3mintemp + q4mintemp
...
m4 <- feols(statetaxSPREAD_MMAinterp ~ only_wns_positive
            + lnsize
            + lnmat
            + BQ
            + GO
...
m5 <-  feols(statetaxSPREAD_MMAinterp ~ only_wns_positive
             + lnsize
             + lnmat
             + BQ
             + GO
...
m6 <-  feols(statetaxSPREAD_MMAinterp ~ only_wns_positive
             + lnsize
             + lnmat
             + BQ
             + GO
...
etable(m1, m2, m3, m4, m5, m6,
       file = paste0( '../../tables_figures_SI/table_SI-8_panel_a.tex'),
       replace = T,

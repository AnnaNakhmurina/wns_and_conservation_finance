#----------------------------------------------------------------
# Figure SI-2 Panel C: Event study robustness checks for own revenues per capita
#
# This script estimates the effect of WNS on total own-source revenues per capita
# using four different specifications: (1) no controls, (2) full controls,
# (3) stacked DiD following Cengiz et al. (2019), and (4) imputation estimator
# following Borusyak et al. (2024). Results are combined into a single event study figure.
#----------------------------------------------------------------
...
plot_no_controls <- feols(total_rev_own_sources_per_capita ~ i(event_year, 0)
                          | year + fips,
                          cluster ~ fips + year,
                          fixef.rm = "singleton",
                          data = data
...
plot <- feols(total_rev_own_sources_per_capita ~ i(event_year, 0)
              + lnpop
              + cum_outbreak_count
...
plot_stacked <- feols(total_rev_own_sources_per_capita ~ i(event_year, 0)
                      + lnpop
                      + cum_outbreak_count
                      + q1mintemp + q2mintemp + q3mintemp + q4mintemp
                      + q1precip + q2precip + q3precip + q4precip
...
model.impute <- did_imputation(data = data,
                               yname = "total_rev_own_sources_per_capita",
...
pdf(file = '../../tables_figures_SI/figure_SI-2_panel_c.pdf', width = 18)
...
ggplot(combined_data, aes(x = term, y = Estimate, color = Model, shape = Model)) +
  ...
  labs(
    y = "Own Revenues, p. c.", x = "Event Year",
    color = NULL,
    shape = NULL
  ) +
  scale_color_manual(
  ...
  geom_hline(yintercept = -149.03, linetype = "dashed", color = "black", size = 2) +
  scale_y_continuous(
    limits = c(-450, 350),
    breaks = c(-400, -300, -200, -149.03, -100, 0, 100, 200),
    labels = c("-400", "-300", "-200", "-149.03", "-100", "0", "100", "200")
  ) +
  theme_minimal() +
  theme(
